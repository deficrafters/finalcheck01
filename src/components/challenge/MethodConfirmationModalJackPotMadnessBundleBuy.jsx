...
const TicketModal = ({
  ...
  const pickedGamesData = useSelector(
    (state) => state.popup.JackPotMadnessTickets
  );

  const Jackpot_Bundle_Ticket_Data = useSelector((state) => state.cart);
  ...
  const onModalRequestClose = () => {
    ...
    setSetselectedSubtoken({ parent: "", index: "", selectedToken: "" });
    setFinal_Toke_Name_Selected("");
    setFinal_Toke_Image_Selected("");
    setFinal_Amount_User_Need_To_Pay("");
  };
  ...
  const getWalletDtaa = () => {
    ...
    try {
      axios
        ...
        .then((acc) => {
          // alert(acc.data.data.Deposit_Wallet)
          setGetData(acc.data.data.Deposit_Wallet);
        })
        .catch((err) => {
          console.log(err);
        });
  ...
  async function handle_Buy_Now() {
    ...
    // ! CHECKING WALLET TYPES

    if (radioValue == "Deposit Wallet") {
      let makeString = JSON.stringify(pickedGamesData);

      axios
        .post("/api/JackpotMadness/PurchaseTicket", {
          id: parseIT.data._id,
          data: makeString,
          hash: "From Wallet",
          radioValue,
          Final_Amount: Final_Amount_User_Need_To_Pay,
        })
        .then((acc) => {
          setIsLoading(false);
      ...
      const result = await DepositFn({
        input: Number(Final_Amount_User_Need_To_Pay).toFixed(8).toString(),
        type: Final_Toke_Name_Selected,
        wallet: address,
        chain: chainId,
      });

      let TakeHash = localStorage.getItem("tempHash");

      localStorage.removeItem("tempHash");
        ...
        let makeString = JSON.stringify([
          {
            ...
            price: Number(Final_Amount_User_Need_To_Pay).toFixed(8).toString(),
            entries: Jackpot_Bundle_Ticket_Data.Data.Ticket_Array.length,
            tickets: Jackpot_Bundle_Ticket_Data.Data.Ticket_Array,
          },
        ]);
  ...
  const getOneValue = (obj) => {
    ...
    console.log({ keyss: obj.selectedToken });
  ...
  const Check_How_Much_Cost_User_Need_To_PAY = (Selected_Token) => {
    ...
    let Amount_In_USDT =
      Jackpot_Bundle_Ticket_Data.Data.Ticket_Array.length *
      Jackpot_Bundle_Ticket_Data.Data.One_Ticket_USDT_Price;
          ...
          <div className='h-full bg-left bg-no-repeat rounded-lg p-4 lg:py-5 w-full flex items-center justify-center flex-col lg:flex-row border-[0.5px] border-gray-600 relative bg-[#000000]'>
            ...
            <div className='w-[500px]'>
              ...
              <div className='border-[0.5px] border-blue-400 w-full rounded-xl p-3 mt-4 bg-[#202020]'>
                ...
                {/* <p className="mt-2 text-xs text-gray-500">{active.desc}</p> */}
              ...
              {radioValue == "Connected Wallet" ? (
                ...
                <>
                  ...
                  >
                    ...
                    {pickedGamesData &&
                      pickedGamesData.length > 0 &&
                      pickedGamesData[0].price}{" "}
                    Minimum In Your Wallet To Continue.
                  </div>
                </>
              )}
              ...
              <div className='flex gap-4 mt-4'>
                ...
                {isLoading ? (
                  ...
                  getData < pickedGamesData[0].price ? (
                  <button
